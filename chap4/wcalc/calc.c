...
char *set_parameters(char *v, int o, int b, int c)
{ 
  Node vars;
  parse_string = v;
  parse_string_index = 0;
  ...
  return show_ring_str(CurrentRing);
}

char *print_mono_str(Monomial m)
{
  ULONG mask,e;
...
char *yyparse_str(char *str)
{
  return print_poly_str(eval_string(str));
}
