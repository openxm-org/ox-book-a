...
{
	char *cbuf;
//	cbuf = (char *)GC_malloc(mpz_sizeinbase(mpq_numref(a.q), 10) + mpz_sizeinbase(mpq_denref(a.q), 10) + 3);
	cbuf = (char *)calloc(mpz_sizeinbase(mpq_numref(a.q), 10) + mpz_sizeinbase(mpq_denref(a.q), 10) + 3,1);
  if ( a.q == 0 ){
    sprintf(cbuf,"0");
  }else{
